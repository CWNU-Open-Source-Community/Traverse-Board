...
| 底部面板 | 显示终端面板布局和明确的“尚未启用”状态 | 当前没有 PTY、xterm 输入或 Shell 启动 |
| 右侧栏 | 切换审阅、终端、浏览器、文件和侧边任务 | 审阅/文件/任务复用既有有界能力；终端和浏览器保持诚实的未启动状态 |
...
The Windows Desktop workbench now exposes four real controls rather than screenshot placeholders: an operator-confirmed Open Workspace menu, a read-only Summary, an inert Bottom Panel, and a Right Sidecar for Review, Terminal, Browser, Files, and Side Tasks. Review, Files, and Side Tasks reuse existing bounded Go capabilities; the embedded Terminal and Browser remain explicitly unavailable.
...
在仓库根目录运行 `powershell -ExecutionPolicy Bypass -File scripts/build-desktop.ps1 -VerifyReproducible`，然后双击 `build/desktop/cyberagent-desktop.exe`。默认启动为只读模式，不会运行 Shell、LocalRunner 或 Docker，也不会写注册表、创建自启动项或安装服务。已有 `.cyberagent-workbench/cyberagent.db` 会在事务内原地升级；应用不会以删除或重置本地数据作为启动恢复手段。2026-07-22 的真实 Windows 11 验收已覆盖 v30 预览库到 v84 的保数据升级、主工作台渲染、设置页交互、优雅关闭和关闭后 SQLite 完整性检查；边界见 ADR 0068。

From the repository root, run `powershell -ExecutionPolicy Bypass -File scripts/build-desktop.ps1 -VerifyReproducible`, then open `build/desktop/cyberagent-desktop.exe`. The default launch is read-only: it does not run Shell, LocalRunner, or Docker and does not write the registry, create startup entries, or install a service. An existing `.cyberagent-workbench/cyberagent.db` is upgraded transactionally in place; startup recovery never deletes or resets local data. The 2026-07-22 Windows 11 acceptance run covered a data-preserving v30-preview-to-v84 upgrade, rendered workbench, Settings interaction, graceful close, and post-close SQLite integrity verification. See ADR 0068.
...
### Windows Desktop (through schema v84 / D1-G12, D1-V11)
