...
- **架构完成度 / Architecture completion：约 99%**。衡量 Go 控制平面、Run/Session、状态恢复、Policy、审批、预算、事件流、Tool Gateway、Agent 协调、Skills、报告、Sandbox 协议及 Go/TypeScript/Rust 边界的覆盖程度；其中 V2 Run-centric Runtime 约 99%。schema v90 在固定命令审批链之外增加独立的任意一次性命令合同、显式完全访问宿主执行账本，以及 Debug 终端 Agent 输入的短租约与元数据审计。
...
- **Architecture completion: about 99%.** Schema v90 adds a separate arbitrary one-shot command contract, a write-ahead full-access host-execution ledger, and short-lived, metadata-audited Debug terminal Agent-input bindings without widening the schema-v89 fixed-command protocol.
...
`execution-permission` 只选择可请求的策略上限，不是通用执行入口。`command-plan` 只生成非启动计划；`command-execute` 是分离的固定模板执行入口；`command-proposal` 只审阅 Agent 已提交的固定申请。`host-execute` 是独立的操作者完全访问入口，不消费模型提案，也不经过 Shell 字符串解析。模型和仓库内容不能代替人工确认。PowerShell 参数本身不是沙箱，低完整性令牌也不是工作区专用文件能力或网络阻断。完整决策见 [ADR 0075](docs/adr/0075-execution-interaction-trust-model.md)、[ADR 0076](docs/adr/0076-controlled-windows-execution-and-user-terminal.md)、[ADR 0077](docs/adr/0077-four-level-run-execution-permissions.md)、[ADR 0079](docs/adr/0079-review-gated-fixed-command-proposals.md) 与 [ADR 0081](docs/adr/0081-gated-host-execution-and-debug-terminal-input.md)。

Prayu separates interaction shape from permission ceiling. Schema v86 records `preview|controlled|debug|cyber` interaction intent; schema v88 records `conservative|approval|full_access|debug` permission policy; schema v89 adds review-gated Agent requests for the same four fixed templates; schema v90 adds a separately gated, operator-only, non-sandboxed one-shot Windows host executor. The arbitrary approval proposal contract and Debug Agent-input controller remain internal Go foundations with no model, HTTP, or renderer route. Selection and persistence never grant runtime authority, and every operation rechecks process-local startup capabilities.
...
下表是唯一按时间排序的 schema 开发历程，完整保留了早期 `v1`、`v2`、`v3`，并连续列到当前 `v90`。这里的 `vN` 是不可变 SQLite schema/runtime 里程碑，不等同于产品发布版本；后面的架构说明按能力域组织，因此不再承担版本排序职责。

The table below is the canonical chronological schema history. It includes every immutable SQLite schema/runtime milestone from `v1` through the current `v90`. These schema numbers are not product release versions; the architecture notes that follow are grouped by capability instead of chronology.
...
Model Harness 与 P13-A 批次没有新增迁移；P12-A1 将 SQLite 推进到 schema v86，P12-B1 推进到 v87，P12-C1-C3 推进到 v88，P12-D1-D3 推进到 v89，P12-E2 再推进到 v90。v89 仍只允许四种固定命令提案，不是任意 Shell。v90 的宿主执行器是默认关闭、双确认、操作者 CLI 专用的 `full_access` 一次性入口；`approval` 任意提案仍只有 Go 合同，Debug Agent 输入仍只有 Go 内部控制器。Docker PTY、真实浏览器启动、Profile 落盘和 CDP 继续关闭。

Model Harness and P13-A add no migration. P12-A1 advances SQLite to v86, P12-B1 to v87, P12-C1-C3 to v88, P12-D1-D3 to v89, and P12-E2 to v90. Schema v89 still permits only four fixed command proposals. Schema v90 adds a default-off, dual-confirmed, operator-only `full_access` Windows one-shot host executor. The arbitrary `approval` proposal remains a Go contract, and Debug Agent input remains a Go-internal controller rather than a model, HTTP, or renderer capability.
...
Desktop D0-A、D0-B、D1-R1 至 D1-G13/V12 与 D1-UX11 自动化核心已完成；项目全局 SQLite 当前为 v90，并包含非产品 R10 Runner 回执黄金边界、不启动的浏览器接纳账本、P12-B 受控命令和用户终端、P12-C 四档权限、P12-D 固定命令提案审批链，以及 P12-E 非沙箱宿主执行账本。项目固定 Wails v2.13.0 稳定版，并提供 Windows `cyberagent-desktop` 开发/便携测试二进制。Vite production bundle 在编译期嵌入；现有 Go `api.v1` Handler 直接接入 Wails AssetServer，不监听 TCP 端口，也不建立第二套业务 API。桌面端默认只读，每类 mutation 都由独立显式 flag 开启；`--enable-command-proposals` 只开放固定提案审批页和一次性受控执行，不开放任意 Shell。`run host-execute` 当前只属于操作者 CLI，Desktop renderer 没有任意命令入口。本地 Monaco、Repository、验证、Handoff/Journey、系统凭证和 wake worker 继续复用既有 Go read/control 边界；`--enable-user-terminal` 只为精确 Code/Local/Debug Run 开放用户所有的 ConPTY/xterm，会话、环境和输出不持久化。Debug Agent 输入控制器只存在 Go 内部，模型和 renderer 不能签发或调用。原生 Acrylic、浅色/深色/透明玻璃主题、CSS/React Prayu 标识和独立权限设置页不增加任何运行权限。通用模型 Shell、Docker PTY 和浏览器仍关闭。
...
The automated Desktop core is now at schema v90 and D1-UX11, including the test-only R10 receipt boundary, the non-starting browser ledger, P12-B controlled-command/user-terminal boundaries, the P12-C permission ceiling, the P12-D fixed-command proposal workflow, and the P12-E host-execution ledger. Wails v2.13.0 embeds the Vite production bundle and connects the existing Go `api.v1` Handler directly to the AssetServer without opening a TCP listener. The shell remains read-first and independently gates mutations. `--enable-command-proposals` enables only fixed-proposal review and restricted one-shot execution; it does not enable arbitrary Shell. The operator-only `run host-execute` command has no Desktop renderer route. `--enable-user-terminal` exposes a user-owned ConPTY/xterm session only for an exact Code/Local/Debug Run; session state, environment, and output are not persisted. Debug Agent input remains a Go-internal controller with no model or renderer route. Native Acrylic, light/dark/transparent-glass appearance, the CSS/React Prayu mark, and the dedicated Permission settings page add no runtime authority. General model Shell, Docker PTY, and the built-in browser remain disabled.
...
以下内容按能力域和当前阅读价值组织，不代表 schema 时间顺序；需要核对开发先后时，请以上方 `v1 -> v90` 表为准。
...
The following notes are organized by capability and current reading value, not by schema order. Use the canonical `v1 -> v90` table above whenever chronology matters.
...
### Windows Desktop (through schema v90 / P12-E and D1-UX11)
