...
- **架构完成度 / Architecture completion：约 99%**。衡量 Go 控制平面、Run/Session、状态恢复、Policy、审批、预算、事件流、Tool Gateway、Agent 协调、Skills、报告、Sandbox 协议及 Go/TypeScript/Rust 边界的覆盖程度；其中 V2 Run-centric Runtime 约 99%。schema v91 在 v90 宿主执行边界之外增加独立的 `restricted|full_debug` 浏览器 CDP 权限快照、进程启动闸门与不可授权审计合同。
- **产品可用度 / Product usability：约 96-98%**。衡量普通用户能否依靠当前 CLI、TUI、Web 和 Windows Desktop 完成真实端到端工作。通用 Coding Agent 工作流约 97%，Cyber 自动化工作流约 20%；四种固定诊断动作已形成 Agent 提案到人工审批的完整链，操作者还可在双重确认和本次进程闸门下从 CLI 执行一次非沙箱宿主命令。任意 `approval` 提案仍只有 Go 合同，Debug Agent 输入仍只有 Go 内部控制器，二者尚无模型、HTTP 或 React 产品入口。网络隔离执行、Docker 持久终端、内置浏览器、安装脚本/钩子、Windows 10 人工发布矩阵和 Cyber 工具链仍未完成。
...
- **Architecture completion: about 99%.** Schema v91 adds an independent `restricted|full_debug` browser-CDP policy ceiling and process-local gates on top of the schema-v90 host-execution boundary, without starting a browser or enabling a CDP transport.
- **Product usability: about 96-98%.** The generic coding-agent workflow is about 97% usable and Cyber automation about 20%. Fixed diagnostics have an end-to-end Agent-proposal workflow, and an operator can explicitly launch one non-sandboxed host command from the CLI under the current process gates. Arbitrary approval proposals and Debug Agent input are still Go-internal foundations with no model, HTTP, or React product route. Network-isolated execution, Docker terminals, the built-in browser, install hooks, the Windows 10 release matrix, and the Cyber toolchain remain unfinished.
...
本批功能门已通过全量 Go 测试、application/HTTP 竞态测试、安全 Desktop tags、48 个前端测试文件共 178 项测试、严格 TypeScript、生产构建与 0 高危 npm 漏洞审计；测试期间未启动浏览器。The delivery gate passed the complete Go suite, focused application/HTTP race tests, secure Desktop tags, 48 frontend files with 178 tests, strict TypeScript, the production build, and a zero-high-severity npm audit; no browser was started during verification.

真实启动所需的前置接纳、权限选择与审计合同已经完成，但真实 start adapter、Job Object 进程树执行、Profile 文件写入、CDP 网络连接和用户可操作窗口仍不存在。边界见 [ADR 0069](docs/adr/0069-go-owned-browser-profiles-target-scope-and-session-plan.md)、[ADR 0071](docs/adr/0071-inert-browser-executable-profile-lifecycle-and-sealed-cdp.md)、[ADR 0073](docs/adr/0073-browser-publisher-launch-lease-and-review-gates.md) 与 [ADR 0082](docs/adr/0082-browser-cdp-permission-ceilings.md)。The prerequisite acceptance, permission, and audit contracts now exist, but the real start adapter, Job Object process execution, profile writes, CDP network connection, and user-facing browser window still do not.
...
Model Harness 与 P13-A 批次没有新增迁移；P12-A1 将 SQLite 推进到 schema v86，P12-B1 推进到 v87，P12-C1-C3 推进到 v88，P12-D1-D3 推进到 v89，P12-E2 推进到 v90，P11-C4A 再推进到 v91。v91 只记录 CDP 能力上限，不启动浏览器或打开 transport；Docker PTY、真实浏览器启动、Profile 落盘和真实 CDP 继续关闭。

Model Harness and P13-A add no migration. P12-A1 advances SQLite to v86, P12-B1 to v87, P12-C1-C3 to v88, P12-D1-D3 to v89, P12-E2 to v90, and P11-C4A to v91. Schema v91 records only a CDP policy ceiling: it neither starts a browser nor enables a transport. Docker PTY, real browser startup, Profile writes, and real CDP remain disabled.
...
Desktop D0-A、D0-B、D1-R1 至 D1-G13/V12 与 D1-UX11 自动化核心已完成；项目全局 SQLite 当前为 v91，并包含非产品 R10 Runner 回执黄金边界、不启动的浏览器接纳账本、P12-B 受控命令和用户终端、P12-C 四档宿主权限、P12-D 固定命令提案审批链、P12-E 非沙箱宿主执行账本，以及独立的受限/完整调试 CDP 权限快照。项目固定 Wails v2.13.0 稳定版，并提供 Windows `cyberagent-desktop` 开发/便携测试二进制。Vite production bundle 在编译期嵌入；现有 Go `api.v1` Handler 直接接入 Wails AssetServer，不监听 TCP 端口，也不建立第二套业务 API。桌面端默认只读，每类 mutation 都由独立显式 flag 开启；`--enable-command-proposals` 只开放固定提案审批页和一次性受控执行，不开放任意 Shell。`--enable-browser-cdp-control` 只开放 CDP 权限选择，`--enable-full-cdp-debug` 还要求 Debug 最大权限；二者都不启动浏览器或打开 CDP transport。`run host-execute` 当前只属于操作者 CLI，Desktop renderer 没有任意命令入口。本地 Monaco、Repository、验证、Handoff/Journey、系统凭证和 wake worker 继续复用既有 Go read/control 边界；`--enable-user-terminal` 只为精确 Code/Local/Debug Run 开放用户所有的 ConPTY/xterm，会话、环境和输出不持久化。Debug Agent 输入控制器只存在 Go 内部，模型和 renderer 不能签发或调用。原生 Acrylic、浅色/深色/透明玻璃主题、CSS/React Prayu 标识和独立权限设置页不增加任何运行权限。通用模型 Shell、Docker PTY 和真实浏览器仍关闭。
...
The automated Desktop core is now at schema v91 and D1-UX11, including the test-only R10 receipt boundary, the non-starting browser ledger, P12-B controlled-command/user-terminal boundaries, the P12-C host permission ceiling, the P12-D fixed-command proposal workflow, the P12-E host-execution ledger, and independent restricted/full-debug CDP permission snapshots. `--enable-browser-cdp-control` exposes only operator selection; `--enable-full-cdp-debug` also requires maximum Debug execution capability. Neither gate starts a browser or opens a CDP transport. General model Shell, Docker PTY, and the operational built-in browser remain disabled.
...
### Windows Desktop (through schema v91 / P11-C4C, P12-E, and D1-UX11)
...
data, or grant product authority. The operational built-in browser remains unavailable until the
separate publisher, same-handle, durable lifecycle, process-tree, and operator-review gates pass.
