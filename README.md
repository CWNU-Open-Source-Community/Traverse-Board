...
2026-08-02 的管理员探针已越过权限、动态 WFP 会话和原子安装，但以 `baseline_canaries_not_observed` 失败。根因是 `MAP * ~NOTFOUND` 同时拦截了探针自己的 IP 字面量。现在 DNS 仍默认拒绝，只为 Go 创建并由 WFP 精确约束的五个本机 canary IP 加入解析豁免；真实 Edge baseline 冒烟测试已在约 0.6 秒内观察到五项请求。该结果不是 WFP 生产验收，仍需重新运行上述管理员命令并取得 `passed=true`。

The 2026-08-02 elevated run crossed the elevation, dynamic-session, and atomic-install gates but failed with `baseline_canaries_not_observed`. `MAP * ~NOTFOUND` was also suppressing the probe's own IP literals. DNS remains default-deny; only the five Go-created local canary addresses, still constrained to exact ports by WFP, are excluded from that resolver rule. A real installed-Edge baseline smoke test now observes all five requests in about 0.6 seconds. This is not production WFP acceptance; the elevated command must still be rerun and return `passed=true`.
