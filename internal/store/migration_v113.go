...
// debugTerminalSupervisorLedgerStatements widens the durable Supervisor call
// ledger to the complete v113 registry. SQLite cannot alter a CHECK constraint
// in place, so the migration rebuilds the table transactionally and copies all
// existing calls before restoring its index and guards.
var debugTerminalSupervisorLedgerStatements = []string{
	...
	`ALTER TABLE run_supervisor_tool_calls RENAME TO run_supervisor_tool_calls_v112;`,
	...
	`INSERT INTO run_supervisor_tool_calls
		...
		payload_json, status, result_json, error_code, created_at, completed_at
		FROM run_supervisor_tool_calls_v112;`,
	`DROP TABLE run_supervisor_tool_calls_v112;`,
