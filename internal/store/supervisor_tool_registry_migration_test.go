...
func TestSchemaV116AndV119PreserveAuthorityAndAdmitRuntimeTools(t *testing.T) {
	ctx := context.Background()
	...
	if _, err := upgraded.db.ExecContext(ctx, `INSERT INTO run_supervisor_tool_calls
		...
		checkpoint.NextTurn, checkpoint.AttemptID, "mcp-tool-v119",
		...
		domain.SupervisorToolPending, ts(time.Now().UTC())); err != nil {
		t.Fatalf("insert mcp_tool_call after v119 migration: %v", err)
	}
	rounds, err := upgraded.ListSupervisorToolRounds(ctx, checkpoint)
	if err != nil || len(rounds) != 1 || len(rounds[0].Calls) != 3 {
		t.Fatalf("v116/v119 Supervisor calls=%#v err=%v", rounds, err)
	}
	if rounds[0].Calls[0].AuthorityJSON != string(encodedAuthority) ||
		...
		t.Fatalf("v116/v119 authority preservation failed: %#v", rounds[0].Calls)
	}
}
