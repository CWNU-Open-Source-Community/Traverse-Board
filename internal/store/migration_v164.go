...
// automaticFileEditMoveAuthorizationStatements extends the immutable Full
// Access authorization source to non-overwriting moves. Historical create and
// replace sources retain their row identity and empty destination binding.
var automaticFileEditMoveAuthorizationStatements = []string{
	`DROP TRIGGER trg_file_edit_auto_authorization_insert;`,
	`DROP TRIGGER trg_file_edit_auto_authorization_update;`,
	`DROP TRIGGER trg_file_edit_auto_authorization_delete;`,
	`DROP TRIGGER trg_file_edit_auto_content_update;`,
	`DROP TRIGGER trg_file_edit_auto_approval_insert;`,
	`DROP TRIGGER trg_file_edit_auto_approval_update;`,
	`DROP TRIGGER trg_file_edit_auto_apply_insert;`,
	`DROP INDEX idx_file_edit_auto_authorizations_run_created;`,
	`ALTER TABLE file_edit_auto_authorizations RENAME TO file_edit_auto_authorizations_v163;`,
	...
	`INSERT INTO file_edit_auto_authorizations
		(edit_id, operation_key_digest, proposal_fingerprint, run_id, session_id,
		 workspace_id, operation_kind, path, destination_path, original_hash,
	 ...
	 SELECT edit_id, operation_key_digest, proposal_fingerprint, run_id, session_id,
		 workspace_id, operation_kind, path, '', original_hash, proposed_hash, '', '',
		 permission_snapshot_id, permission_revision, mode_revision, runtime_epoch,
		 runtime_generation, agent_id, capability_generation, lease_id,
		 lease_generation, created_at
	 FROM file_edit_auto_authorizations_v163;`,
	`DROP TABLE file_edit_auto_authorizations_v163;`,
