...
func TestCommonCapabilitySkillBodiesPreserveEvidenceAndAuthorityBoundaries(t *testing.T) {
	...
	contracts := map[string][]string{
		...
		"review":          {"complete merge-base diff", "stable hunk graph", "base/ours/theirs object IDs", "unexpected worktrees", "retry, skip, abort, restart", "confirmed, inferred, or unverified", "github_review_evidence_read", "untrusted data", "never turn an omitted page"},
		"focused-checks":  {"complete merge-base diff", "stable hunk identity", "continue/skip/abort", "bounded bisect", "checkpoint or recovery receipt", "must never be reported as passed", "GitHub review evidence", "missing or queued check"},
